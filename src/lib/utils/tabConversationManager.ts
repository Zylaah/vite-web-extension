/**
 * Tab Conversation Manager for Hana extension
 * Based on SolBrowse's TabConversationManager pattern
 */
...
export class TabConversationManager {
  ...
  private constructor() {
    ...
    this.conversationKey = `hana-tab-conversation-${this.tabId}`;
  ...
  setConversation(messages: TabConversation['messages'], conversationId: string | null): void {
    try {
      sessionStorage.setItem(this.conversationKey, JSON.stringify({ messages, conversationId }));
    } catch (error) {
      console.error('Hana: Failed to save tab conversation:', error);
    }
  }
  ...
  clearConversation(): void {
    try {
      ...
      console.error('Hana: Failed to clear tab conversation:', error);
    }
  }
  ...
  private setupNavigationListeners(): void {
    const handleNavigation = () => {
      ...
      if (newUrl !== this.currentUrl || newHost !== this.currentHost) {
        console.log(`Hana TabConversationManager: Navigation detected: ${this.currentHost} -> ${newHost}`);
        ...
        this.navigationHandlers.forEach(handler => {
          try {
            ...
            console.error('Hana: Error in navigation handler:', error);
          }
        });
      }
    };
